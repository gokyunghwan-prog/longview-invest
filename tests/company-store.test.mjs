...
test("쿼리 파서는 허용값과 상한을 검증한다", () => {
  ...
  assert.throws(() => query("country=JP"), CompanyQueryError);
  assert.throws(() => query("country=ALL"), CompanyQueryError);
...
test("목록은 서버 필터·안정 정렬·페이지네이션을 적용하고 상세 필드를 제외한다", async (t) => {
  ...
  assert.deepEqual(overview.meta.coverage, { total: 3, kr: 3 });
  assert.equal(overview.meta.sync.failed, 0);
  assert.equal("errors" in overview.meta.sync, false);
  assert.equal("countries" in overview.facets, false);
  ...
  const filtered = store.list(
    query("q=%EF%BC%A1%EF%BC%AC%EF%BC%B0%EF%BC%A8%EF%BC%A1&sector=정보기술")
  );
  assert.equal(filtered.pagination.total, 1);
  assert.equal(filtered.items[0].id, "KR-000001");
...
test("10,000개 데이터도 한 페이지의 경량 응답만 반환한다", async (t) => {
  const companies = Array.from({ length: 10_000 }, (_, index) =>
    company(index + 1, {
      ...
      id: (index % 2 === 0 ? "KR-" : "JP-") + String(index + 1).padStart(6, "0")
    })
  );
  const temporary = await temporaryDataset(companies);
  t.after(() => rm(temporary.directory, { recursive: true, force: true }));
  const store = await createCompanyStore(temporary.file, STORE_OPTIONS);
  const result = store.list(query("page=50&pageSize=50"));
