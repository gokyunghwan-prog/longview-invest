...
test("공개된 전체 순위 안에서만 현재 자금에 맞는 상위 종목을 목표로 삼는다", () => {
...
test("같은 공개 순위에서도 계좌자금이 커지면 고가의 상위 종목을 편입한다", () => {
  const companies = Array.from({ length: 6 }, (_, index) =>
    company(`KR-CAPITAL-${index + 1}`, {
      sector: `sector-${index + 1}`,
      score: 96 - index
    })
  );
  const prices = [40_000, 67_000, 45_000, 8_000, 9_000, 10_000];
  const quotes = Object.fromEntries(
    companies.map((item, index) => [
      item.id,
      { ...quoteFor(item), price: prices[index] }
    ])
  );
  const selection = {
    status: "ready",
    strategyVersion: INVESTMENT_SELECTION_STRATEGY_VERSION,
    policy: { preferredPositions: 3 },
    ranked: companies.map((item, index) => ({
      id: item.id,
      ticker: item.ticker,
      country: item.country,
      exchange: item.exchange,
  ...
  const config = { strategy: { version: INVESTMENT_SELECTION_STRATEGY_VERSION } };

  const small = selectPublishedPortfolio({
    companies,
    quotes,
  ...
  const large = selectPublishedPortfolio({
    companies,
    quotes,
    totalEquityKrw: 1_000_000,
    selection,
    config,
    now,
    incumbents: [companies[3]]
  });

  assert.deepEqual(
    small.selected.map((item) => item.investmentRank),
    [4, 5, 6]
  );
  assert.deepEqual(
    large.selected.map((item) => item.investmentRank),
    [1, 2, 3]
  );
});
