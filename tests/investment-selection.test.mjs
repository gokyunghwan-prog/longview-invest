...
function build(companies) {
  return buildInvestmentSelection({
    ...
    modelVersion: "2.0.0",
    generatedAt: NOW,
    policy: { referenceCapitalKrw: 100_000 }
  });
}
...
test("기본 공개 예시는 100만원 기준으로 고가 상위후보도 선정한다", () => {
  const artifact = buildInvestmentSelection({
    companies: [
      scoredCompany("KR-HIGH", { score: 95, price: 67_000, sector: "업종-HIGH" }),
      scoredCompany("KR-A", { score: 92, price: 40_000, sector: "업종-A" }),
      scoredCompany("KR-B", { score: 90, price: 45_000, sector: "업종-B" }),
      scoredCompany("KR-C", { score: 88, price: 9_000, sector: "업종-C" })
    ],
    sourceRevision: "revision-1",
    sourceUpdatedAt: "2026-07-20T10:00:00.000Z",
    modelVersion: "2.0.0",
  ...
  assert.equal(artifact.policy.referenceCapitalKrw, 1_000_000);
  assert.deepEqual(
    artifact.selected.map((item) => item.id),
    ["KR-HIGH", "KR-A", "KR-B"]
  );
});
...
test("참조자금으로 서로 다른 3개 업종을 만들 수 없으면 fail-closed 한다", () => {
  ...
  assert.equal(artifact.policy.referenceCapitalKrw, 100_000);
});
