...
> 현재 운영 구조: 웹사이트의 `자동투자 선정순`과 실제 자동매매는 같은 공개 선정 파일을 사용합니다. GitHub Actions가 매일 데이터·순위를 갱신하고, 평일 09:23 KST에 KIS 실전 리밸런싱, 15:13 KST에 주문 대조를 수행하도록 구현되어 있습니다. 실전 예약은 안전을 위해 저장소 변수 `AUTOTRADE_LIVE_ENABLED=true`를 설정하기 전까지 비활성입니다. 자세한 설정과 중지 방법은 [자동투자 운영 안내](./docs/AUTOTRADE.md)를 보세요.
...
현재 자동투자 정책은 국내 적격 종목만 대상으로 목표 현금을 0%로 두고, 최초 배치에서는
가용자금을 최대한 투자합니다. 이후에는 매일 새 평가를 확인하지만 비중 이탈, 더 우수한
...
5. 성공한 `data/companies.json`만 자동 커밋
