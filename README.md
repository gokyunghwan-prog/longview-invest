...
> 현재 운영 구조: 웹사이트의 `자동투자 선정순`과 실제 자동매매는 같은 공개 선정 파일을 사용합니다. GitHub Actions가 저녁부터 세 번 데이터·순위 갱신을 시도하고, 평일에는 30분 간격의 멱등 heartbeat 중 09:05~14:50 KST에 도착한 실행만 KIS 실전 리밸런싱을 수행합니다. 15:13 KST에는 주문 대조를 별도로 시도합니다. 실전 예약은 안전을 위해 저장소 변수 `AUTOTRADE_LIVE_ENABLED=true`를 설정하기 전까지 비활성입니다. 자세한 설정과 중지 방법은 [자동투자 운영 안내](./docs/AUTOTRADE.md)를 보세요.
