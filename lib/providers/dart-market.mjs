...
const DEFAULT_DART_CASH_FLOW_ENRICHMENT_MAX_DURATION_MS = 45 * 60_000;
const DART_CASH_FLOW_PARSER_VERSION = 1;
...
async function fetchDartCashFlowStatement({
  ...
  for (const basis of requestedBases) {
    ...
    if (!Array.isArray(payload?.list)) {
      sawUnparsedRows = true;
      continue;
    }
    if (payload.list.length === 0) continue;
    const record = normalizeDartCashFlowStatement(payload.list, {
      corpCode: company.corpCode,
      statementBasis: basis,
      businessYear,
      reportCode
    });
    if (hasUsefulDartCashFlowRecord(record)) {
      return { outcome: "DATA", record };
    }
    sawUnparsedRows = true;
  }
  return {
    outcome: sawUnparsedRows ? "UNPARSED" : "NO_DATA",
    record: null
  };
}
...
function dartCashFlowResultMetadata({
  outcome,
  annualAccounts,
  record = null,
  fetchedAt
  ...
  return {
    outcome,
    parserVersion: DART_CASH_FLOW_PARSER_VERSION,
    businessYear: String(annualAccounts.businessYear || record?.businessYear || ""),
    reportCode: String(annualAccounts.reportCode || record?.reportCode || ""),
    statementBasis: String(
      annualAccounts.statementBasis || record?.statementBasis || ""
    ),
    currency: normalizedCurrency(annualAccounts.currency || record?.currency),
    sourceFilingId:
      String(annualAccounts.filingId || record?.filingId || "").trim() || null,
    fetchedAt
  };
}

function createEmptyDartCashFlowCache() {
  return {
    schemaVersion: 3,
    parserVersion: DART_CASH_FLOW_PARSER_VERSION,
...
function dartCashFlowCacheShapeIsValid(cache) {
  if (!isPlainJsonObject(cache)) return false;
  if (
    cache.schemaVersion !== undefined &&
    !Number.isInteger(cache.schemaVersion)
  ) {
    return false;
  }
  if (cache.periodKey !== undefined && typeof cache.periodKey !== "string") {
    return false;
  }
  return [
    "recordsByCorpCode",
    ...
    (field) =>
      cache[field] === undefined || isPlainJsonObject(cache[field])
  );
}

async function readDartCashFlowCache(cacheFile) {
  try {
    const parsed = JSON.parse(await readFile(cacheFile, "utf8"));
    if (dartCashFlowCacheShapeIsValid(parsed)) {
      return { cache: parsed, resetRequired: false };
    }
    return { cache: createEmptyDartCashFlowCache(), resetRequired: true };
  } catch (error) {
    if (error?.code === "ENOENT") {
      return {
        cache: createEmptyDartCashFlowCache(),
        resetRequired: false
      };
    }
...
function dartCashFlowCacheIsFresh({
  ...
  const hasStoredRecord = Object.prototype.hasOwnProperty.call(
    cache.recordsByCorpCode || {},
    corpCode
  );
  const record = hasStoredRecord
    ? cache.recordsByCorpCode[corpCode]
    : undefined;
  const resultMetadata = cache.resultMetadataByCorpCode?.[corpCode] || null;
  if (
    cache.completedCorpCodes?.[corpCode] !== true ||
    !resultMetadata ||
    resultMetadata.parserVersion !== DART_CASH_FLOW_PARSER_VERSION
  ) {
    return false;
  }
  const outcome = String(resultMetadata.outcome || "");
  if (
    (outcome === "DATA" && !hasUsefulDartCashFlowRecord(record)) ||
    (outcome === "NO_DATA" && (!hasStoredRecord || record !== null)) ||
    !["DATA", "NO_DATA"].includes(outcome)
  ) {
    return false;
  }
  if (
    String(resultMetadata.businessYear || "") !==
      String(annualAccounts.businessYear || "") ||
    String(resultMetadata.reportCode || "") !== "11011" ||
    String(resultMetadata.statementBasis || "") !==
      String(annualAccounts.statementBasis || "") ||
    normalizedCurrency(resultMetadata.currency) !==
      normalizedCurrency(annualAccounts.currency)
  ) {
    return false;
  }
  if (
    outcome === "DATA" &&
    (String(record.businessYear || "") !==
      String(annualAccounts.businessYear || "") ||
      String(record.reportCode || "") !== "11011" ||
      String(record.statementBasis || "") !==
        String(annualAccounts.statementBasis || "") ||
  ...
  const expectedFilingId = String(annualAccounts.filingId || "").trim();
  const cachedFilingId = String(
    resultMetadata.sourceFilingId ||
      cache.sourceFilingIdsByCorpCode?.[corpCode] ||
      record?.filingId ||
      ""
  ).trim();
  if (
    ...
    (!expectedFilingId ||
      !cachedFilingId ||
      cachedFilingId !== expectedFilingId)
  ) {
    return false;
  }
  if (
  ...
  const fetchedAt = new Date(
    resultMetadata.fetchedAt || cache.fetchedAtByCorpCode?.[corpCode] || ""
  ).getTime();
...
async function syncDartCashFlowStatements({
  ...
  maxDurationMs = DEFAULT_DART_CASH_FLOW_ENRICHMENT_MAX_DURATION_MS,
  elapsedTimeMs = () => performance.now(),
  onProgress = () => {}
}) {
  const periodKey = `${businessYear}:${reportCode}`;
  const loadedCache = await readDartCashFlowCache(cacheFile);
  let cache = loadedCache.cache;
  let cacheResetRequired = loadedCache.resetRequired;
  if (cache.periodKey !== periodKey || ![2, 3].includes(cache.schemaVersion)) {
    cache = createEmptyDartCashFlowCache();
    cacheResetRequired = true;
  } else if (cache.schemaVersion === 2) {
    cache.resultMetadataByCorpCode = {};
    for (const [corpCode, completed] of Object.entries(
      cache.completedCorpCodes || {}
    )) {
      if (completed !== true) continue;
      const record = cache.recordsByCorpCode?.[corpCode];
      if (!hasUsefulDartCashFlowRecord(record)) {
        delete cache.completedCorpCodes[corpCode];
        continue;
      }
      const fetchedAt = cache.fetchedAtByCorpCode?.[corpCode] || null;
      cache.resultMetadataByCorpCode[corpCode] = {
        outcome: "DATA",
        parserVersion: DART_CASH_FLOW_PARSER_VERSION,
        businessYear: String(record.businessYear || ""),
        reportCode: String(record.reportCode || ""),
        statementBasis: String(record.statementBasis || ""),
  ...
  cache.resultMetadataByCorpCode ??= {};
  cache.schemaVersion = 3;
  cache.parserVersion = DART_CASH_FLOW_PARSER_VERSION;
  cache.source = "Open DART fnlttSinglAcntAll";
  cache.periodKey = periodKey;
  cache.businessYear = String(businessYear);
  cache.reportCode = String(reportCode);
  if (cacheResetRequired) {
    cache.updatedAt = new Date(currentTime).toISOString();
    await writeJsonAtomic(cacheFile, cache);
  }
  ...
  const completedEligibleCount = () =>
    eligibleCompanies.filter(
      (company) => cache.completedCorpCodes[company.corpCode] === true
    ).length;
  const startedAt = Number(elapsedTimeMs());
  const durationLimit = Number.isFinite(Number(maxDurationMs))
    ? Math.max(0, Number(maxDurationMs))
    : DEFAULT_DART_CASH_FLOW_ENRICHMENT_MAX_DURATION_MS;
  ...
  for (const company of eligibleCompanies) {
    const annualAccounts = annualAccountsByCorpCode[company.corpCode];
    if (
    ...
    const elapsed = Number(elapsedTimeMs()) - startedAt;
    if (Number.isFinite(elapsed) && elapsed >= durationLimit) {
      cache.updatedAt = new Date(currentTime).toISOString();
      cache.completedThisRun = completedThisRun;
      cache.deadlineReached = true;
      cache.deadlineReachedAt = new Date(currentTime).toISOString();
      cache.incompleteCorpCount =
        eligibleCompanies.length - completedEligibleCount();
      delete cache.completedAt;
      await writeJsonAtomic(cacheFile, cache);
      onProgress(
        `DART 연간 현금흐름 제한시간 도달 · 체크포인트 ${completedEligibleCount()}/${eligibleCompanies.length} 저장 후 계속`
      );
      return cache;
    }
    try {
      const cashFlowResult = await fetchDartCashFlowStatement({
        controller,
        company,
        businessYear,
        reportCode,
      ...
      const { outcome, record } = cashFlowResult;
      const fetchedAt = new Date(currentTime).toISOString();
      cache.recordsByCorpCode[company.corpCode] =
        outcome === "DATA" ? record : null;
      cache.fetchedAtByCorpCode[company.corpCode] = fetchedAt;
      cache.sourceFilingIdsByCorpCode[company.corpCode] =
        annualAccounts.filingId || record?.filingId || null;
      cache.resultMetadataByCorpCode[company.corpCode] =
        dartCashFlowResultMetadata({
          outcome,
          annualAccounts,
          record,
          fetchedAt
      ...
      if (outcome === "UNPARSED") {
        ...
        cache.errorsByCorpCode[company.corpCode] =
          "DART 현금흐름 행을 현재 파서로 해석하지 못했습니다.";
      } else {
      ...
      delete cache.completedCorpCodes[company.corpCode];
      delete cache.resultMetadataByCorpCode[company.corpCode];
    }
    completedThisRun += 1;
    ...
    if (changedSinceCheckpoint >= checkpointEvery) {
      ...
      onProgress(
        `DART 연간 현금흐름 ${completedEligibleCount()}/${eligibleCompanies.length}`
      );
    }
  }
  ...
  cache.incompleteCorpCount =
    eligibleCompanies.length - completedEligibleCount();
  if (cache.incompleteCorpCount === 0) {
    cache.completedAt = new Date().toISOString();
  } else {
    delete cache.completedAt;
  }
  cache.completedThisRun = completedThisRun;
  cache.deadlineReached = false;
  delete cache.deadlineReachedAt;
  await writeJsonAtomic(cacheFile, cache);
  return cache;
}
...
export async function syncDartMarket(config, options = {}) {
  ...
  if (cashFlowEnabled && annualPeriod) {
    ...
    try {
      cashFlowCache = await syncDartCashFlowStatements({
        ...
        maxDurationMs:
          options.cashFlowMaxDurationMs ??
          DEFAULT_DART_CASH_FLOW_ENRICHMENT_MAX_DURATION_MS,
        elapsedTimeMs:
          options.cashFlowElapsedTimeMs || (() => performance.now()),
        onProgress
      });
    } catch (error) {
  ...
  const dataset = {
    meta: {
      ...
      fcfCoverage,
      cashFlowSyncError,
      cashFlowDeadlineReached: Boolean(cashFlowCache.deadlineReached),
...
export function getDartMarketApiLimits() {
  return {
    ...
    defaultCashFlowEnrichmentMaxDurationMs:
      DEFAULT_DART_CASH_FLOW_ENRICHMENT_MAX_DURATION_MS
  };
}
