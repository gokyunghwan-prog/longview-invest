...
class DartRequestController {
  ...
  async json(endpoint, parameters, { allowNoData = false, retries = 2 } = {}) {
    let lastError;
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      await this.beforeRequest(endpoint);
      try {
        const payload = await this.fetchJsonImpl(endpointUrl(endpoint, this.apiKey, parameters));
...
async function syncUniverse({
  ...
  const corpRecords = await controller.external("corpCode.xml", () => fetchCorpCodes(apiKey));
...
async function syncFinancialBatches({
  ...
  businessYear,
  reportCode,
  batchSize = MAX_DART_COMPANIES_PER_REQUEST,
  onProgress = () => {}
}) {
...
export function buildDartMarketCompany({
  ...
  const errors = [universeEntry.overviewError, financialError].filter(Boolean);
...
export async function syncDartMarket(config, options = {}) {
  ...
  for (const period of financialPeriods) {
    const cache = await syncFinancialBatches({
      ...
      batchSize: options.batchSize || MAX_DART_COMPANIES_PER_REQUEST,
      onProgress
    });
  ...
  const dataset = {
    meta: {
      ...
      note: "Open DART 공식 공시 기반 전체 KOSPI·KOSDAQ 발행회사 데이터입니다. 현금흐름 지표는 전체 재무제표 보강 전까지 결측으로 유지합니다."
    },
    companies
  };
