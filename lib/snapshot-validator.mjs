...
export function validateSnapshot(snapshot, options = {}) {
  ...
  if (marketCoverage && typeof marketCoverage === "object") {
    ...
    const actualUs = companies.filter((company) => isFresh(company, "US")).length;
    if (!numberEquals(marketCoverage.kr, actualKr)) {
      addError(`meta.marketData.coverage.kr=${marketCoverage.kr} does not match ${actualKr}.`);
    }
    if (!numberEquals(marketCoverage.us, actualUs)) {
      addError(`meta.marketData.coverage.us=${marketCoverage.us} does not match ${actualUs}.`);
    }
    for (const [bucket, predicate] of [
      ...
      for (const [country, key] of [["KR", "kr"], ["US", "us"]]) {
        const actual = companies.filter(
          (company) => company.country === country && predicate(company.marketData)
        ).length;
        if (!numberEquals(declared[key], actual)) {
          addError(`meta.marketData.${bucket}.${key}=${declared[key]} does not match ${actual}.`);
        }
      }
    }
  }
