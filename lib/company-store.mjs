...
export function parseCompanyQuery(searchParams) {
  ...
  if (requestedCountry !== null && String(requestedCountry).toUpperCase() !== "KR") {
    throw new CompanyQueryError("국가 필터는 국내 기업(KR)만 지원합니다.");
  }
...
function buildSnapshot(raw, scoringDate = new Date()) {
  if (!raw || !Array.isArray(raw.companies)) {
    throw new Error("회사 데이터셋 형식이 올바르지 않습니다.");
  }

  const koreanSnapshot = normalizeKoreanSnapshot(raw);
  const safeScoringDate = Number.isNaN(scoringDate.getTime()) ? new Date() : scoringDate;
  const companies = scoreAndRank(koreanSnapshot.companies, safeScoringDate);
  ...
  const revision = createHash("sha256")
    .update(JSON.stringify(koreanSnapshot))
    .digest("hex")
    .slice(0, 20);
  const meta = sanitizedMeta(koreanSnapshot.meta);
  return {
    revision,
    meta,
    companies,
...
export class CompanyStore {
  ...
  async performReload() {
    try {
      ...
      const nextSnapshot = buildSnapshot(raw, scoringNow);
