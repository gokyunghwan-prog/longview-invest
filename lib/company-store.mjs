...
const LIST_COMPONENTS = ["profitability", "growth", "safety", "cashflow"];
...
function listProjection(company) {
  ...
  if (projectedMetrics.per === null && Number.isFinite(valuation.per)) {
    projectedMetrics.per = valuation.per;
  }
  ...
  return {
    ...
    score: {
      ...
      components,
      band: company.score?.band || { key: "cautious", label: "주의" },
      candidate: {
...
function evaluationPriority(entry) {
  ...
  if (company.modelApplicability === false || status === "not_applicable") return 2;
  if (
...
function buildSnapshot(raw, rawText) {
  if (!raw || !Array.isArray(raw.companies)) {
    throw new Error("회사 데이터셋 형식이 올바르지 않습니다.");
  }

  const referenceDate = new Date(raw.meta?.updatedAt || Date.now());
  const scoringDate = Number.isNaN(referenceDate.getTime()) ? new Date() : referenceDate;
  const companies = scoreAndRank(raw.companies, scoringDate);
  ...
  return {
    ...
    overview: {
      ...
      summary: {
        ...
        averageScore: average(companies, (company) => company.score.total),
        averageConfidence: average(companies, (company) => company.score.dataConfidence)
      },
...
export class CompanyStore {
  ...
  async performReload() {
    try {
      ...
      const nextSnapshot = buildSnapshot(raw, rawText);
